...
* thin
* coderay
* mixlib-log (1.1.0) from github opscode/mixlib-log
* mixlib-authentication from github opscode/mixlib-authentication
...
rake gem
install all gems in the pkg directories of each chef-* directory
...
initialize a new client key on the server
  knife client create <foo>
  copy ~/.chef/ to client host
